...
//*****************物体模型************************
glm::vec3 modelPos;
//Mesh myModel("C:\\Users\\dell-pc\\Desktop\\test.obj");
vector<Mesh> meshes;
...
void sceneInit()
{
	...
	//myModel = ImportedModel("C:\\Users\\dell-pc\\Desktop\\test.obj")
...
{
	Mesh model("C:\\Users\\dell-pc\\Desktop\\untitled.obj");
	meshes.push_back(model);
	Mesh model2("C:\\Users\\dell-pc\\Desktop\\test.obj");
...
void passOne(void)
{
	...
	for (size_t i = 0; i < meshes.size(); i++)
	{
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo[i][3]);
		size_t partCounts = meshes[i].getMeshParts().size();
		for (size_t j = 0; j < partCounts; j++)
		{
			size_t lastCount = 0;
			lastCount += (j == 0 ? 0 : meshes[i].getMeshParts()[j - 1].inds.size());
			glDrawElements(GL_TRIANGLES, meshes[i].getMeshParts()[j].inds.size(), GL_UNSIGNED_INT, (void *)(4 * lastCount));
			glDrawElements(GL_TRIANGLES, meshes[i].getNumIndicates(), GL_UNSIGNED_INT, (void*)(4 * lastCount));
		}
...
void passTwo(void)
{
	...
	for (size_t i = 0; i < meshes.size(); i++)
	{
		...
		for (size_t j = 0; j < partCounts; j++)
		{
			if (meshes[i].getMeshParts()[j].material.hasDiffTex)
			{
				tex1 = meshes[i].getMeshParts()[j].material.textureDiffuse;
			}
			else
			{
				tex1 = texEmpty;
			}
			if (meshes[i].getMeshParts()[j].material.hasBumpTex)
			{
				tex2 = meshes[i].getMeshParts()[j].material.textureBump;
			}
			else
			{
				tex2 = texEmpty;
